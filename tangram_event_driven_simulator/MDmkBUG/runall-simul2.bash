...
K="1 1"  # initial and final number of servers
...
alloptions=('0.0004400677458 0.01847404397 0.000009237021984 0.99')
...
do
	      ...
	      options=${alloptions[i]}
				...
				do
					...
					echo -e "\n" >> options
					cp template.obj MDmk$i.obj
					sed -ibak 's/MAX_SERVERSCONST/'$i'/g' MDmk$i.obj
					sed -ibak 's/PAC_RATECONST/'$PAC_RATECONST'/g' MDmk$i.obj
					sed -ibak 's/SERVICE_TIME1CONST/'`bc -l <<< "$SERVICE_TIME1CONST /$i"`'/g' MDmk$i.obj
					sed -ibak 's/SERVICE_TIME2CONST/'`bc -l <<< "$SERVICE_TIME2CONST /$i"`'/g' MDmk$i.obj
					sed -ibak 's/ALPHACONST/'$ALPHACONST'/g' MDmk$i.obj
				

			#		sed -i bak 's/INITIALPOPULATION/'$i'/g' MDmk$i.maxvalues

		
					make compile BASE=MDmk$i ADDITIONAL=$i
					make simul BASE=MDmk$i ADDITIONAL=$i
					make clean BASE=MDmk$i
					zip out$i.zip *.out
					rm *.out -rf
